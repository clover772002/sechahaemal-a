...
CRITERIA = [
    "3일 중 강수확률 60% 이상이 하루라도 있으면 빨간불 가중",
    "3일 중 비 예보가 2일 이상이면 빨간불",
    "3일 평균 강수확률 40% 이상이면 노란불 이상",
    "초미세먼지 예상이 '나쁨' 이상이면 노란불 이상",
    "3일 모두 강수확률 30% 미만 + 초미세먼지 '보통' 이하면 초록불",
]
...
def evaluate_car_wash(rain_summary: dict, dust_forecast: dict) -> dict:
    ...
    if max_pop >= 60:
        score -= 35
        reasons.append(f"3일 중 최대 강수확률이 {max_pop}%로 높습니다.")
    if rainy_days >= 2:
        score -= 30
    ...
    if avg_pop >= 40:
        score -= 15
        reasons.append(f"3일 평균 강수확률이 {avg_pop}%입니다.")
    elif avg_pop <= 20:
        score += 5
        reasons.append(f"3일 평균 강수확률이 {avg_pop}%로 낮습니다.")

    if worst_dust >= 4:
        score -= 25
        reasons.append("초미세먼지 예상이 매우 나쁩니다.")
    elif worst_dust >= 3:
        score -= 15
        reasons.append("초미세먼지 예상이 나쁩니다.")
    elif worst_dust <= 2:
        score += 5
        reasons.append("초미세먼지 예상이 양호합니다.")
