...
| `KAKAO_REST_API_KEY` | 카카오 **REST API 키** (세차장 검색, **권장**) |

4. **Deploy** 클릭

### 카카오 세차장 검색 (권장)

카카오맵과 비슷한 결과를 내려면 Vercel에 **REST API 키**를 넣습니다.

1. [developers.kakao.com](https://developers.kakao.com) → 앱 → **앱 키** → **REST API 키** 복사
2. **제품 설정 → 로컬** API 활성화
3. Vercel **Environment Variables** → `KAKAO_REST_API_KEY`에 붙여넣기 ( `NEXT_PUBLIC_` 붙이지 마세요 )
4. **Redeploy** (환경 변수 변경 후 필수)

(대안) `NEXT_PUBLIC_KAKAO_JAVASCRIPT_KEY` + Web 도메인 등록으로 브라우저 SDK 검색도 가능합니다.
...
| `KAKAO_REST_API_KEY` | `frontend/.env.local` | **Vercel** (세차장 검색, 권장) |
| `NEXT_PUBLIC_KAKAO_JAVASCRIPT_KEY` | `frontend/.env.local` | Vercel (대안) |
